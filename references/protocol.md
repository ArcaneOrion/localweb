...
默认 session 是 `cli-main`，为了兼容旧项目仍使用 `.localweb/state.json`、`.localweb/events.jsonl` 和 `.localweb/inbox/events.jsonl`。显式传入 `--session <id>` 时，状态和输入流写入 `.localweb/sessions/<id>/`。

`panels/`、`shell/` 和 `assets/` 是项目级共享资源；session 隔离的是当前 UI 快照、事件审计流和 Web 到 CLI 的 inbox。端口不是身份边界，端口上的 `serve` 实例只是观察某个 session 的本地入口。
...
- `session_id` 是当前 UI 快照所属的 LocalWeb session；多开端口时应使用 `--session` 把不同 CLI 工作流隔离。
...
`events.jsonl` 是当前 session 的 agent 到 Web 事件流和审计历史。每行都是一个 JSON 对象，至少包含 `type` 和 `ts` 字段。

`inbox/events.jsonl` 是当前 session 的 Web 到 CLI 低风险上下文输入流。底部辅助选择事件格式：
...
# 多 session 场景：wait 必须使用发布输入时相同的 session
...
```json
{
  ...
  "command_status": "ok",
  "wait_required": true,
  "wait": {"id": "next", "type": "choice", "session_id": "review-auth"},
  "next_command": "uv run scripts/localweb.py wait --session review-auth --id next"
}
```
...
`serve` 启动时会在 `.localweb/runtime/servers/<server_id>.json` 写入本地服务实例信息：`server_id`、`host`、`port`、`url`、`pid`、`session_id`、`started_at` 和 `heartbeat_at`。`doctor` 会读取这些记录并标注进程是否仍在运行。

server registry 只用于发现和诊断多开端口，不是状态真相源。真实 UI 状态仍来自对应 session 的 `state.json`，浏览器输入仍写入对应 session 的 inbox。
