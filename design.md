...
Web companion = 可视化投影、结构化阅读面、轻量选择面
```
...
- CLI 会话上下文为唯一主上下文。
- Web 端只展示 agent 生成的 HTML panel 和轻量选择。
...
`state.json` 表示当前 UI 快照。Web shell 读取它，决定显示哪个 panel、顶部状态、左侧上下文和底部选择按钮。
...
`inbox/events.jsonl` 是 Web 到 CLI 的轻量输入管道。

Web 点击按钮后写入：

```jsonl
{"type":"choice","choice_id":"next","value":"B","label":"看源码路径","session_id":"codex-main","ts":"2026-06-05T17:40:20+08:00"}
```

agent CLI 只有在显式运行 `localweb wait --id next` 后，选择结果才进入 CLI 输出，进而进入模型上下文。
...
uv run scripts/localweb.py choice --id next --option A="看架构" --option B="看示例"
...
| `choice` | 创建 A/B/C/D 等选择项 |
| `wait` | 阻塞等待 Web 点击，返回选择值 |
...
```text
┌─[ LOCALWEB COMPANION // HTML DECK ]───────────────────────────────────────┐
│ STATUS: WAITING USER   SESSION: codex-main   PIPE: READY   PANEL: main    │
├──────────────────────┬───────────────────────────────────────────────────┤
│ CONTEXT STACK         │                                                   │
│                       │             LIVE HTML STAGE                       │
│  01 当前任务           │                                                   │
...
│     A / B / C / D     │      └────────────────────────────────────┘       │
│                       │                                                   │
│  04 主会话             │      [ A 看整体流程 ] [ B 看源码路径 ]             │
│     CLI terminal      │      [ C 做练习题   ] [ D 返回目录   ]             │
└──────────────────────┴───────────────────────────────────────────────────┘
```
...
- panel 可以是学习图、代码图、对比表、流程图、报告、编辑器。
- shell 不解释 panel 内容，只负责承载、刷新和收集轻量选择。
- 不使用大面积柔和渐变，不做软糯卡片堆叠。
- 线条硬，颜色高对比，但信息密度保持清晰。
...
用户点击 Web
  ...
  -> localweb wait --id next 返回 B
  -> CLI agent 看到 B
  -> 模型继续推理
```

这保证 Web 选择不会成为“隐形上下文”。
...
- Web 点击 A/B/C/D 后写入 inbox。
- `localweb wait` 能返回对应选择。
- 服务默认只监听 `127.0.0.1`。
...
- Web 点击如果没有显式 wait，用户可能以为已经进入 CLI 上下文。
- 文件协议若过早复杂化，会拖慢 MVP。
...
- 顶部明确显示 pipe 状态：`READY / WAITING / SENT`。
- `wait` 返回后在 `events.jsonl` 记录 consumed 状态。
- 第一版只支持 choice，不支持任意命令执行。
